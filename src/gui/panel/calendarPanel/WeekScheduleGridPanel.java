...
public class WeekScheduleGridPanel extends RoundedPanel {
	...
	private static final Color CURRENT_DAY_BORDER = new Color(0x2F, 0x80, 0xA9);
	...
	public WeekScheduleGridPanel(GUIInterface guiInterface, LocalDate weekStart, LocalDate indicatorDay,
		...
		setBorder(BorderFactory.createLineBorder(GRID_BORDER_COLOR, 1));
	...
	private JPanel buildGrid() {
		...
		grid.setOpaque(true);
		grid.setBackground(GRID_BORDER_COLOR);
	...
	private JPanel buildDayHeaderCell(LocalDate day) {
		...
		JPanel cell = buildGridCell(currentDay ? CURRENT_DAY_BACKGROUND : Color.WHITE, 76);
		if (currentDay) {
			cell.setBorder(BorderFactory.createMatteBorder(0, 0, 2, 1, CURRENT_DAY_BORDER));
		}
	...
	private JPanel buildSlotLabelCell(String text, int preferredHeight) {
		JPanel cell = buildGridCell(Color.WHITE, preferredHeight);
	...
	private JPanel buildGridCell(Color background, int preferredHeight) {
		...
		cell.setBorder(BorderFactory.createMatteBorder(0, 0, 1, 1, GRID_BORDER_COLOR));
